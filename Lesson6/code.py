...
def game_scene():
    ...
    def show_alien():
        ...
        for alien_number in range(len(aliens)):
            aliens[alien_number].move(random.randint(2 * constants.SPRITE_SIZE, constants.SCREEN_X - 2 * constants.SPRITE_SIZE),
                                      random.randint(2 * constants.SPRITE_SIZE, constants.SCREEN_Y - 2 * constants.SPRITE_SIZE))
            break
    ...
    game.layers = bullets + [ship] + aliens + [background]
    ...
    while True:
        ...
        # A button to fire with button states
        if keys & ugame.K_X != 0:
        ...
        for alien_number in range(len(aliens)):
            ...
            if aliens[alien_number].x > ship.x:
                aliens[alien_number].move(aliens[alien_number].x - constants.ALIEN_SPEED, aliens[alien_number].y)
            ...
            # Stay if the same
            if aliens[alien_number].x == ship.x:
                aliens[alien_number].move(aliens[alien_number].x, aliens[alien_number].y)
            # Stay if the same
            if aliens[alien_number].y == ship.y:
                aliens[alien_number].move(aliens[alien_number].x, aliens[alien_number].y)
